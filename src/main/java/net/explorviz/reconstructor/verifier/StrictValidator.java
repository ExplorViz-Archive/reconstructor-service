...
/**
 * Validator tha enforces that all values of the {@link EVSpan} are set and valid.
 * Does not replace invalid attributes with default values.
 */
public class StrictValidator implements SpanValidator {

  @Override
  public EVSpan validate(EVSpan span) throws InvalidSpanException {
  ...
  private void validateTimestamp(EVSpan span) throws InvalidSpanException {
    try {
      Instant ignored = Instant.ofEpochSecond(span.getStartTime().getSeconds(), span.getStartTime().getNanoAdjust());
